...
sub get_makefile_include_template {
	...
	Util::WARN_AND_DIE("In HAsim, root module must specify three Makefile templates.");
    }
...
sub get_makefile_template {
	...
	Util::WARN_AND_DIE("No " . $idx . "-th makefile template in module with " . num_makefile_templates($module) . " templates.");
    }

    return Asim::resolve(Util::path_append($module->base_dir(),$templates[$idx]));
}
